...
function generateFolder(program, cwd, folder, fileName) {
    ...
    if (configName) {
        var isConfigPathNoExists = _fs.existsSync(configPath);
        console.log(isConfigPathNoExists, 'isConfigPathNoExists')
        _assert(isConfigPathNoExists, `警告:${configPath}不存在`); //配置文件是否存在
        if (!isConfigPathNoExists) {
            //拿到数据,进行渲染
